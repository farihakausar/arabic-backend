...
const signup = async (req, res) => {
  try {
    const { fullName, username, email, mobileNumber, password, role } =
      req.body;
    ...
    if (role === "artist") {
      ...
      res
        .status(201)
        .json({ message: "Artist created successfully", artistId: artist._id });
    } else if (role === "patron") {
      // Save to User schema
      const patronProfile = new PatronProfile(userData);
      await patronProfile.save();
      res.status(201).json({
        message: "patronProfile created successfully",
        userId: patronProfile._id,
      });
    } else {
